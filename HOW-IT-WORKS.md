...
### How a draft pick is graded

A pick is worth **what it returned above a startable player**, measured against
**what that draft slot normally returns**:

    par   = max(0, his points - the last startable player at his position)
    score = par - DRAFT_CURVE[the slot he was taken at]

DRAFT_CURVE is the *median* points above replacement each slot has returned
across every season on file, regenerated by `scripts/gen-draft-curve.mjs` and
checked by `test-draft.mjs`.

**A late miss costs nothing, and no rule says so.** From about pick 46 onward
more than half of all picks return a player who never starts, so the curve is
zero there, so whiffing on a round-8 flier scores exactly 0. A late *hit* still
banks everything it produced.

This replaced a positional rank delta (drafted RB4, finished RB1, +3), which
had two faults that compounded: a rank point is not worth the same everywhere
(slipping three spots from RB1 costs 54 real points, from RB60 costs 2.7, and
both scored -3), and an early pick could not win — the first player taken at a
position had a ceiling of exactly zero. Over four seasons the best any round-1
pick ever scored was +4 while a round-13 flier scored +47, and rounds 7+
...
move a team score by about 3% of its swing.

**What the score is not.** It says how a draft turned out, not how well someone
drafted. Across four seasons the year-over-year correlation of a manager's own
draft score is 0.212 — and a permutation test that reshuffles who got which
pick lands the real spread inside the 5th-95th percentile of pure chance in
every season. There are real outliers every year; they are different people
every year.
